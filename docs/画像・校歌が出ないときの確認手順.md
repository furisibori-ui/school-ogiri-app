...
| `Comet image API non-OK:` | 4xx/5xx が返った | Comet ダッシュボードでエラー内容・レート制限を確認。**404 で「models/xxx は見つからないか generateContent ではサポートされていません」** → モデル ID が無効。Vercel の環境変数 **COMET_IMAGE_MODEL** に、Comet の利用可能モデル（例: `gemini-2.5-flash`）を設定して再デプロイ。 |
