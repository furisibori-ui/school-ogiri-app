...
## 2'. 一部の画像だけ出ない・セクションごと欠落する場合（8枚中2枚だけなど）

**校章と現校舎は出るが、校長・初代校舎・行事・部活・銅像・制服の画像（とセクション）が出ない** —— この場合は **Step1（テキスト生成）の戻り値が途中で切れている**可能性が高いです。

### なぜそうなるか

- 画像は **Step2** で「Step1 で返ってきたデータのうち、`image_prompt` がありかつ `image_url` がプレースホルダーのもの」だけをタスクにしている。
- Step1 の JSON が **途中で切れている**（max_tokens 不足や API タイムアウト）と、`principal_message`・`historical_buildings[0]`・`multimedia_content.school_events`・`club_activities`・`monuments`・`uniforms` などが **欠落または空** になる。
- すると **画像タスクがそもそも 2 個しか作られず**（校章・現校舎だけ）、ほかは「データがない」ので画像も生成されず、UI ではそのセクションごと非表示になる。

### 確認するログ

1. **Vercel → Logs** で、該当の「学校生成」実行時刻を指定し、次のログを検索する：
   - **`step2 collectImageTasks`** … `taskCount` と `types` が出る。ここが **2** や **3** なら「Step1 のデータが欠けているため、元から画像タスクが少ない」。
   - **`step2 done`** … `imageCount` と `realImages`。imageCount が 2 のままなら、タスクが 2 個しかない。
2. **Inngest → Runs** でその Run を開き、**step1-text-and-anthem** の **Step Output** を見る。  
   - `principal_message`、`multimedia_content.school_events` / `club_activities` / `monuments` / `uniforms`、`school_profile.historical_buildings` が **無い・空配列** なら、Step1 の応答が途中で切れている。

### 対処

- **max_tokens を増やす**: `app/api/generate-school/route.ts` の `maxTokens`（Comet）や `max_tokens`（Anthropic）を **12288 以上** にし、長い JSON が最後まで返るようにする（すでに 12288 なら、さらに増やすかプロンプトを少し短くする）。
- **テキスト生成のタイムアウト**: 245 秒で打ち切られている場合は、Comet の応答が遅いか負荷で遅延している可能性。時間をおいて再試行するか、プロンプト・リサーチ量を減らす。
- 詳細は [現状まとめ_テキストAPI連携.md](./現状まとめ_テキストAPI連携.md) の「サイトの後半が欠ける」を参照。
...
| **`Suno fetch returned HTML for task_id=`** | **タスク照会（ポーリング）**で Comet が JSON ではなく HTML を返している。認証エラー・エンドポイント違い・利用制限の可能性 | Comet の Suno **照会 API**（`GET /suno/fetch/{task_id}`）の URL・認証方法をドキュメントで確認。API キーが有効か、Suno が契約に含まれているか確認。 |
...
| `Suno poll timeout` | ポーリングで完了までに至らなかった（60 秒以内に完了しなかった） | Suno 側の遅延・混雑。時間をおいて再試行。Comet の照会 API が HTML を返している場合は上記「Suno fetch returned HTML」を参照。 |
