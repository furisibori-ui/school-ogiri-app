...
| **`Suno fetch returned HTML for task_id=`** | **タスク照会（ポーリング）**で Comet が JSON ではなく HTML を返している。認証エラー・エンドポイント違い・利用制限の可能性 | Comet の Suno **照会 API**（`GET /suno/fetch?task_id=...` または `/suno/fetch/{task_id}`）の URL・認証方法をドキュメントで確認。API キーが有効か、Suno が契約に含まれているか確認。HTML が返り続けると 300 秒で Inngest がタイムアウトするため、コードではポーリングを 24 秒で打ち切るようにしている。 |
...
| `Suno poll timeout` | ポーリングで完了までに至らなかった（24 秒以内に完了しなかった） | Suno 側の遅延・障害。時間をおいて再試行。Comet の照会 API が HTML を返している場合は上記「Suno fetch returned HTML」を参照。 |
