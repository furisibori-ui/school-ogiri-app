...
**※ 後半のセクション（生徒心得・行事・部活・施設・銅像・制服・沿革・教員）がまとめて無い場合**  
→ テキスト生成の **出力トークン上限（max_tokens）** で途中打ち切られている可能性が高い。時間切れではない。  
→ `app/api/generate-school/route.ts` の `max_tokens` を 8192 などに増やす。詳しくは `docs/現状まとめ_テキストAPI連携.md` の「サイトの後半が欠ける場合」を参照。
