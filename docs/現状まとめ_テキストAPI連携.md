...
## ⚠️ サイトの後半が欠ける・JSONが途中で切れる場合（まず疑うこと）

**原因はほぼ「出力トークン上限（max_tokens）」です。時間切れではありません。**

- テキスト生成APIには **max_tokens**（モデルが返していい最大トークン数）がある。
- このアプリでは **8192** に設定している（以前は 2048 で、ここで切れていた）。
- **2048 のとき**：校名・校長・校歌・news あたりまで書くとトークンが尽き、**crazy_rules・行事・部活・施設・銅像・制服・教員・沿革などが書けずに打ち切り**になる。生成時間は 20 秒程度でも「途中で止まった」ように見える。
- **打ち切りは「245秒タイムアウト」の前**に、**出力が max_tokens に達したタイミング**で起こる。

**確認場所**: `app/api/generate-school/route.ts` の `maxTokens` / `max_tokens`（Comet と Anthropic の両方）。足りない場合は 8192 や 4096 に増やす。
